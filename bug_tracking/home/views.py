    ...
    if request.user.is_authenticated:
        ...
        if not request.user.is_superuser:
            ...
            if request.user.is_staff:
                ...
                reportes_usuario_id = reportes_usuario.values_list('id_reporte', flat=True)
                reportes_solucionados = Bug.objects.filter(id_bug__in=reportes_usuario_id, estado="SOLUCIONADO").count()

                reportes_asignados = Bug.objects.filter(id_bug__in=reportes_usuario_id, estado="ASIGNADO").count()
                reportes_enproceso = Bug.objects.filter(id_bug__in=reportes_usuario_id, estado="EN PROCESO").count()
                context = {
